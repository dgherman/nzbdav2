...
public static class SqliteTargetWriter
{
    ...
    public sealed class TargetWriteSession : IDisposable
    {
        ...
        public void Commit()
        {
            ...
            // So the check has to run BEFORE Commit(), still inside the pending transaction,
            // where the rows are still visible to `PRAGMA foreign_key_check`. Round 22: this
            // logic is now shared with GuidCasingRepair.Run() via ForeignKeyDiagnostics, rather
            // than duplicated - see that class's doc comment.
            //
            // Deliberately don't call Commit() at all if this throws - throwing here (rather
            // than letting a real commit attempt fail) means Dispose()'s existing
            // dispose-without-commit path handles the rollback exactly the same way every other
            // pre-commit failure in this session already does, so this diagnostic can never
            // itself write partial data or change the safe-rollback behavior already in place.
            ForeignKeyDiagnostics.ThrowIfAnyViolations(_conn, _tx);
