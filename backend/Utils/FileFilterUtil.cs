...
/// <summary>
/// Filename-based filtering shared by the queue post-processors and the
/// webdav collections: sample detection and glob matching.
/// </summary>
public static partial class FileFilterUtil
{
    ...
    {
        return SampleNameRegex().IsMatch(filename);
    }
    ...
    public static bool IsSampleFile(string filename, long? fileSize, long largestVideoFileSize)
    {
        ...
        if (!LooksLikeSampleName(Path.GetFileName(filename))) return false;
