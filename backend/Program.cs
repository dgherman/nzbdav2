...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        Log.Warning("═══════════════════════════════════════════════════════════════");
        Log.Warning("  NzbDav Backend Starting - BUILD v2026-09-12-PROVIDER-CANCELLATION-RETRY");
        Log.Warning("  FIX (provider cancellation freeze): Planet Earth III S01E02 on Synology returned 20 prematurely truncated buffered ranges in one playback, including four in the final seven minutes where video visibly froze while audio continued. Each ended with an unfilled ordering slot but no terminal segment error. A provider-local OperationCanceledException could escape while the stream and job tokens were both live; the worker mislabeled it as straggler-monitor preemption, assumed the monitor had queued a replacement, and silently dropped the dequeued segment. Provider-local cancellation is now retried in the normal per-segment loop, and the worker defensively re-queues any unrequested cancellation that still escapes. A deterministic regression verifies complete byte-exact output after the provider cancels its first attempt.");
