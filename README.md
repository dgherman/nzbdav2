...
*   **Maintenance**: single-flighted the forced gen2 GC under OOM pressure (concurrent workers each triggered their own blocking compacting collect), removed a dead stopwatch reset and a redundant `Array.Clear`, and added regression tests covering the pump leak, the EOF hang, the pool double-release, the wasted tail pipeline and the paused-player teardown.
