...
## v0.12.10 (2026-09-10)
*   **Fix**: (#35) The Express frontend compressed proxied backend responses because `app.use(compression())` had no filter, so media streams and JSON from `/view`, the WebDAV file endpoints, `/api` and `/metrics` were gzip/br-encoded — dropping the upstream `Content-Length` and forcing `Transfer-Encoding: chunked`, which breaks HTTP range requests and seeking (Jellyfin `.strm` Direct Play would buffer the whole file). Compression now skips those proxied paths (`shouldCompress` in `frontend/server-compression.ts`), and the backend additionally sends `Content-Encoding: identity` on `/view` (`GetWebdavItemController`) and the WebDAV GET/HEAD file path (`GetAndHeadHandlerPatch`) so an upstream proxy will not re-compress them.
