...
*   **Tooling**: The vendored `backend/Libs/SharpCompress` projects now audit only their direct package references (`NuGetAuditMode=direct`). A NuGet advisory published after 2026-09-05 for the *transitive* `Microsoft.Build.Tasks.Git` 8.0.0 (via `Microsoft.SourceLink.*`) was failing `dotnet restore` with `NU1902` under those projects' `TreatWarningsAsErrors`, which broke the Docker build. Direct-dependency advisories still surface as warnings.
