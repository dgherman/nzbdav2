...
*   **Fix**: (#36) The Maintenance → Connection Management buttons returned a 404 (surfaced in the UI as `Unexpected token '<'`) because `flatRoutes()` only mounts immediate children of `app/routes/`, and `app/routes/settings/maintenance/reset-connections.tsx` was nested colocation that never registered — the client `POST /settings/maintenance/reset-connections` hit the SSR 404 handler. The route file moved to `app/routes/settings.maintenance.reset-connections/route.tsx` (flat-routes dot convention) so it now mounts at the URL the client already calls.
*   **Fix**: (#37) Editing the SABnzbd **User Agent** field never enabled the Save button because `api.user-agent` was the one written key missing from `isSabnzbdSettingsUpdated`'s dirty check. Added the comparison; the value already persisted correctly once the form was dirty by any other means.
*   **Tooling**: Bumped `Microsoft.SourceLink.GitHub` `8.0.0` → `10.0.401` for the vendored `backend/Libs/SharpCompress` subtree. NuGet advisory GHSA-23fw-v26w-5fgq (published 2026-08-11, updated 2026-09-08) flags the transitive `Microsoft.Build.Tasks.Git` `8.0.0` that SourceLink `8.0.0` pulls in; after the 2026-09-08 update it began failing `dotnet restore` as `NU1902` under that subtree's `TreatWarningsAsErrors`, breaking the Docker build. `10.0.401` carries `Microsoft.Build.Tasks.Git` / `Microsoft.SourceLink.Common` `10.0.401`, which are not flagged. No auditing was disabled — transitive auditing stays fully on for that subtree, and the application's existing dependency advisories (`NU1903` on `Microsoft.OpenApi` and `SQLitePCLRaw.lib.e_sqlite3`) are unaffected and still reported.
...
*   **Reliability**: `RarUtil.GetRarHeaders` now fails fast with a distinct `RarStreamMisalignedException` when a volume's `MarkHeader` is followed by anything other than an `Archive`/`Crypt` header, or when a password-protected volume's parse blows up *after a clean Archive header specifically* (RAR5's `Crypt` header is unencrypted metadata read successfully regardless of password correctness, so a wrong-password `CryptographicException` — thrown at the `Archive` header, the first thing actually decrypted — is never relabeled) — both remaining cases mean the byte stream is misaligned rather than the archive corrupt or the password wrong. Previously SharpCompress would walk to end-of-stream, costing up to the 60-second header-parse timeout per volume.
*   **Reliability**: `InterpolationSearch.Find` (used by `NzbFileStream`, `MultipartFileStream` and `SevenZipProcessor` to locate a byte offset's segment via real yEnc-header probes) now caps itself at 64 probe rounds instead of iterating unbounded — a pathological segment-size table can no longer fan a single seek out into hundreds of HEAD-equivalent fetches; it fails fast with the existing `SeekPositionNotFoundException` instead.
...
*   **Fix**: The prefetch window is no longer raised to `stream buffer + connections` when that exceeds the budget. That figure is a **segment** count and ignores segment size, so on a 4 MB-segment release it meant 90 segments — 360 MB in flight against a 96 MB budget. Measured on production 2026-07-28: four such streams sat at a 359 MB ceiling each while the process ran at 3.2–3.9 GB of a 4 GB heap, i.e. the budget bounded the releases that were never the problem and not the ones that were. The window is now the budget converted for that file, raised only as far as parallelism needs — one segment per connection, which costs `connections × segment size` instead of the whole computed window. A 4 MB-segment stream drops from 300 MB to 100 MB on a 512 MB heap. The log's `source=` now reads `byte-budget` or `parallelism-floor` so the binding constraint is visible.
...
*   **Tooling (`POST /api/gc-diagnostics`, off unless `NZBDAV_GC_DIAG=1`)**: forces a blocking compacting gen2 collection and reports generation sizes on either side of it. `last_collection_heap_size` is the only trustworthy heap metric — `GC.GetTotalMemory` and `dotnet_total_memory_bytes` both count uncollected garbage — but it is a snapshot taken *at the last collection*, so when the process goes idle the collections stop and the metric freezes at whatever the busy state was. Measured on production: 90 seconds after playback stopped, gen2 was still pinned at 13 and LOH still reported its mid-playback 2.71 GB, making the idle floor unreadable. The before/after pair separates rooted memory from garbage awaiting pressure, which is the difference between a leak and a non-problem. It stalls every thread while it runs; reach for it deliberately, do not poll it.
